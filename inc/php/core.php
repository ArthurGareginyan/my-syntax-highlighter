...
function spacexchimp_p010_settings_link( $links ) {
    $page = '<a href="' . admin_url( 'options-general.php?page=' . SPACEXCHIMP_P010_SLUG ) . '">' . __( 'Settings', SPACEXCHIMP_P010_TEXT ) . '</a>';
...
function spacexchimp_p010_register_submenu_page() {
    ...
    $page_title  = SPACEXCHIMP_P010_NAME;
    $menu_title  = __( 'Syntax Highlighter', SPACEXCHIMP_P010_TEXT );
    $capability  = 'manage_options';
    $menu_slug   = SPACEXCHIMP_P010_SLUG;
 ...
 */
function spacexchimp_p010_admin_footer_text() {

    // Get current screen data
    $current_screen = get_current_screen();

    // Return if the page is not a settings page of this plugin
    $settings_page = 'settings_page_' . SPACEXCHIMP_P010_SLUG;
    if ( $settings_page != $current_screen->id ) return;

    // Filter footer text
    function spacexchimp_p010_new_admin_footer_text() {
        $year = date('Y');
    ...
    add_filter( 'admin_footer_text', 'spacexchimp_p010_new_admin_footer_text', 11 );
}
add_action( 'current_screen', 'spacexchimp_p010_admin_footer_text' );

/**
 * Runs during the plugin activation
 */
function spacexchimp_p010_activation() {
    ...
    if ( $activation_date == '' ) {
        ...
        update_option( SPACEXCHIMP_P010_SETTINGS . '_service_info', $info );
    }
}
register_activation_hook( SPACEXCHIMP_P010_FILE, 'spacexchimp_p010_activation' );
...
register_uninstall_hook( SPACEXCHIMP_P010_FILE, 'spacexchimp_p010_uninstall' );
